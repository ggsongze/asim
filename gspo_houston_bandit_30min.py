...
class HoustonGSPOBandit:
    ...
    def _rollout_workday_closed_loop(
        ...
        *,
        ...
        if not result:
            raise RuntimeError("Failed to roll out closed-loop workday in Houston GSPO bandit environment.")
        return result
    ...
    def _rollout_workday(
        ...
        *,
        ...
        system.add("logging:progress").start().wait()
        if not result:
            raise RuntimeError("Failed to roll out workday in Houston GSPO bandit environment.")
        return result
    ...
    def _rollout_workday_requery_planner(
        ...
        *,
        ...
        system.add("logging:progress").start().wait()
        if not result:
            raise RuntimeError("Failed to roll out rolling planner workday in Houston GSPO bandit environment.")
        return result
    ...
    def evaluate_completion(
        ...
        *,
        ...
        if not result:
            raise RuntimeError(
                "Failed to evaluate completion in Houston GSPO bandit environment."
            )
        return result
    ...
    BLOCK_ENV_STEPS = 18       # 3h / 10min = 18 steps per block
    KNOT_ENV_STEPS = 3         # 30min / 10min = 1 env step per knot
    KNOTS_PER_BLOCK = 6        # 3h / 30min = 6 knots = 18 knots
    ...
    def _rollout_block_with_replay(
        ...
        *block_actions* is an 18-element list (one action dict per env step) for the current block.
        ...
        system.add("logging:progress").start().wait()
        if not result:
            raise RuntimeError("Failed to roll out block in Houston GSPO bandit environment.")
        return result
    ...
    def _expand_knots_to_env_steps(
        ...
        *,
        ...
        Each knot is repeated KNOT_ENV_STEPS (3) times.

        When allow_partial=True, accepts fewer than KNOTS_PER_BLOCK knots
        (e.g. last block of the day may be cut short by EP clock jitter
        at the control window boundary).
        """
        ...
        if not allow_partial and len(actions) != self.BLOCK_ENV_STEPS:
            raise ValueError(
                f"Expected {self.BLOCK_ENV_STEPS} env steps from {self.KNOTS_PER_BLOCK} knots, got {len(actions)}"
            )
        return actions
    ...
    def _rollout_block_rolling(
        ...
        *,
        ...
        def _on_timestep(_):
            ...
            cursor = state["action_cursor"]
            same_day = ts.date() == state["target_date"]
            block_done = state["block_env_step"] >= self.BLOCK_ENV_STEPS
        ...
        system.add("logging:progress").start().wait()
        if not result:
            raise RuntimeError("Failed to roll out rolling block in Houston GSPO bandit environment.")
        return result
        ...
        *,
        ...
        for block_index, (block_start_time, block_end_time) in enumerate(self.BLOCK_DEFINITIONS):
            ...
            winner_block_actions_flat = self._expand_knots_to_env_steps(
                winner_knots, allow_partial=True,
            )
