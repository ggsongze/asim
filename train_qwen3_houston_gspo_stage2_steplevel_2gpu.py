...
def main() -> None:
    ...
    _tool_format = os.environ.get("ASIM_TOOL_FORMAT", "json").strip().lower()
    _use_cached = bool(int(os.environ.get("ASIM_USE_CACHED_BACKEND", "0")))
    if _tool_format == "xml":
        if _use_cached:
            from llm_setpoint_planner_cached import CachedQwen35TransformersSamplingBackend
            _BackendCls = CachedQwen35TransformersSamplingBackend
        else:
            from llm_setpoint_planner_qwen35 import Qwen35TransformersSamplingBackend
            _BackendCls = Qwen35TransformersSamplingBackend
    else:
        if _use_cached:
            from llm_setpoint_planner_cached import CachedTransformersSamplingBackend
            _BackendCls = CachedTransformersSamplingBackend
        else:
            _BackendCls = TransformersSamplingBackend
    backend = _BackendCls(
        model=model,
        tokenizer=tokenizer,
        model_name=args.model_name_or_path,
