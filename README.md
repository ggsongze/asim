...
### 2026-04-30 补充：Qwen3.5 v15 checkpoint-8 07:00-19:00 eval

同一口径下复跑 `qwen35_v15_setpoint_only` 的 `checkpoint-8`：

| 项 | 值 |
|---|---|
| LLM checkpoint | `result/gspo/qwen35_9b_v15_vllm_20260428_2112/checkpoint-8` |
| 控制窗口 | **07:00-19:00** |
| Baseline setpoint | **23.0°C cooling** |
| Eval 口径 | `2025-08-25` 原天气 + `2025-08-26..29` swap27 |
| Step / knots | 10-min step, 72 knots/day |

结果：

| Date | Weather | skip_valid_steps | GRPO rel | PPO rel | Δ vs PPO | GRPO day | Baseline day | HVAC kWh | PMV viol |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 2025-08-25 | normal | 1152 | +0.5393 | +0.3844 | +0.1549 | -2.9557 | -3.4950 | 875.6 | 0.230 |
| 2025-08-26 | swap27 | 1224 | +0.5856 | +0.3017 | +0.2839 | +0.5206 | -0.0650 | 820.9 | 0.000 |
| 2025-08-27 | swap27 | 1296 | +1.1985 | +0.5162 | +0.6823 | -12.2512 | -13.4497 | 677.1 | 0.000 |
| 2025-08-28 | swap27 | 1368 | +0.7382 | +0.2000 | +0.5383 | +0.5353 | -0.2029 | 739.8 | 0.005 |
| 2025-08-29 | swap27 | 1440 | +0.5521 | +0.2057 | +0.3463 | -0.1140 | -0.6660 | 747.7 | 0.530 |
| **Total / Mean** | — | — | **+3.6138 / +0.7228** | **+1.6081 / +0.3216** | **+2.0057 / +0.4011** | — | — | — | — |

关键结论：`checkpoint-8` 在完整 5 天 held-out + swap27 口径下明显超过 PPO reference，平均 `+0.7228/day` vs PPO `+0.3216/day`。8/29 的 PMV violation 为 `0.530`，但 reward 仍高于 PPO，主要来自 HVAC/energy 侧收益。

输出文件：
- `result/comparisons/eval_grpo_10min_aug25_checkpoint-8.json`
- `result/comparisons/eval_grpo_10min_aug26_29_swap27_checkpoint-8.json`

### 2026-05-01 补充：Qwen3.5 v15 checkpoint-16 与 base model 对比

`checkpoint-16` 出来后，用同一套 07:00-19:00 / `RL_W_ENERGY=3.0` / baseline 23°C / Aug25 normal + Aug26-29 swap27 口径复跑。结果显示继续从 checkpoint-8 训到 checkpoint-16 **没有带来可见提升**，五天结果与 checkpoint-8 基本一致。

| Date | Weather | GRPO ckpt16 rel | Base model rel | ckpt16 - Base | ckpt16 HVAC kWh | Base HVAC kWh | ckpt16 PMV | Base PMV |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 2025-08-25 | normal | +0.5393 | +0.4874 | +0.0519 | 875.6 | 875.5 | 0.230 | 0.340 |
| 2025-08-26 | swap27 | +0.5856 | +0.5745 | +0.0111 | 820.9 | 816.6 | 0.000 | 0.280 |
| 2025-08-27 | swap27 | +1.1985 | +1.1693 | +0.0292 | 677.1 | 678.1 | 0.000 | 0.000 |
| 2025-08-28 | swap27 | +0.7382 | +0.7691 | -0.0308 | 739.8 | 738.6 | 0.005 | 0.020 |
| 2025-08-29 | swap27 | +0.5521 | +0.8502 | -0.2981 | 747.7 | 746.6 | 0.530 | 0.000 |
| **Total / Mean** | — | **+3.6138 / +0.7228** | **+3.8505 / +0.7701** | **-0.2368 / -0.0474** | **3861.1** | **3855.3** | **0.765** | **0.640** |

7-18 子窗口（按之前 step plot 的口径：前 66 个 10-min step，去掉 18:00-19:00 最后 6 step）：

| Date | ckpt16 rel 7-18 | Base rel 7-18 | ckpt16 - Base |
|---|---:|---:|---:|
| 2025-08-25 | +0.5440 | +0.4488 | +0.0952 |
| 2025-08-26 | +0.5846 | +0.5631 | +0.0215 |
| 2025-08-27 | +1.2000 | +1.1724 | +0.0275 |
| 2025-08-28 | +0.7134 | +0.7594 | -0.0460 |
| 2025-08-29 | +0.5389 | +0.8336 | -0.2948 |
| **Total / Mean** | **+3.5808 / +0.7162** | **+3.7774 / +0.7555** | **-0.1966 / -0.0393** |

8/29 诊断：base model 的优势主要不是来自 PV 或显著更低能耗，而是 **舒适性控制更稳**。13:22 单步 base 比 ckpt16 多拿约 `+0.185` reward：ckpt16 全区设定点 `25.0°C`，PMV violation `0.470`；base 设定点 `23.5-24.5°C`，PMV violation `0.000`。同时 base 在早上/傍晚更敢升温省电，例如 08:22、09:12、17:13 都比 ckpt16 更少过冷。

输出文件：
- `result/comparisons/eval_grpo_10min_aug25_checkpoint-16.json`
- `result/comparisons/eval_grpo_10min_aug26_29_swap27_checkpoint-16.json`
- `result/comparisons/eval_qwen35_base_aug25_7_19.json`
- `result/comparisons/eval_qwen35_base_aug26_29_swap27_7_19.json`
